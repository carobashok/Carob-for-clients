...
def _render_comments(ticket_id, profile, supabase):
    ...
    with st.form(f"comment_form_{ticket_id}"):
        comment_text = st.text_area("Add a comment", height=80, placeholder="Type your comment here...")
        is_internal  = False
        if role in ("agent", "team_lead", "admin"):
            is_internal = st.checkbox("🔒 Internal note (not visible to employee)")
    ...
    with col2:
        ...
        try:
            ...
            if agents:
                ...
                selected_agent = st.selectbox("Assign to agent", agent_names, index=default_idx)
