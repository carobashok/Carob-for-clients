...
a FADA table titled something like "Tractor OEM", "Three-Wheeler OEM", \
"Two-Wheeler OEM", "Passenger Vehicle OEM", or "Commercial Vehicle OEM".
...
- "category" must be EXACTLY one of: "2W", "3W", "PV", "CV", "TRAC", "CE" — inferred
  from the table's title (e.g. "Tractor OEM" -> "TRAC", "Three-Wheeler OEM" -> "3W").
...
def parse_oem_with_claude(pdf_text: str) -> dict:
    ...
    if "category" not in data or "current_fiscal_year" not in data or "rows" not in data:
        raise ValueError(f"Unexpected response shape: {data}")

    data["category"] = normalize_category(data["category"])
    if data["category"] not in OEM_CATEGORIES:
        raise ValueError(f"Unrecognized category in response: {data['category']}")
